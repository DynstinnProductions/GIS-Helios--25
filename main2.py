...
# initialising inputs for user to set work and break durations
input_frame = tk.Frame(root, bg="#000000")
input_frame.place(relx=0.5, rely=0.4, anchor="center")

label_style = {"font": FONT_LABEL, "fg": ACCENT_COLOR, "bg": "#000000"}
...
durations_push = ttk.Button(input_frame, text="Set Work Duration", command=lambda: get_durations(), style="Rounded.TButton")

# Global variables to store durations
work_duration = 0
break_duration = 0

entry_style = {"font": FONT_BUTTON, "fg": "#000000", "justify": "center", "width": 15, "relief": "flat"}

task_label = tk.Label(root, text="Task Reminder", font=FONT_LABEL, fg=MAIN_COLOR, bg="#000000")
task_label.place(x=1600, y=100)

task_entry = tk.Entry(root, font=FONT_BUTTON, fg="#000000", justify="left", width=20, relief="flat")
task_entry.place(x=1600, y=150)

task_button = tk.Button(root, text="Enter Task", font=FONT_BUTTON, fg="#000000", bg=MAIN_COLOR, command= lambda: task(), relief="flat")
task_button.place(x=1600, y=200)

list_label = tk.Label(root, text="Task List", font=FONT_LABEL, fg=MAIN_COLOR, bg="#000000")
list_label.place(x=1600, y=270)

clear_all = tk.Button(root, text="Clear All", font=CLEAR_ALL, fg=MAIN_COLOR, bg="#000000", command=lambda: task_listbox.delete(0, tk.END), relief="flat")
clear_all.place(x=1770, y=277)

task_listbox = tk.Listbox(root, font=LISTBOX, fg=MAIN_COLOR, bg="#000000", width=25, height=40, relief="flat")
task_listbox.place(x=1600, y=300)
...
Welcome_label = tk.Label(root, text="Welcome to FocusZen", font=FONT_HEADING, fg=MAIN_COLOR, bg="#000000")
...
def app_state_1():
    global work_duration, break_duration, timer_running, timer_id
    work_duration_input.delete(0, 'end')
    break_duration_input.delete(0, 'end')
    timer_running = False    
    result_label.config(text="")
    work_duration_input.pack(side="top", padx=15, pady=4)
    work_label.pack(side="top", pady=(0, 3))
    break_duration_input.pack(side="top", padx=15, pady=4)
    break_label.pack(side="top", pady=(0, 3))
    durations_push.pack(side="top", padx=15, pady=5)
...
def get_durations():
    ...
    try:
        work_value = float(work_text)
        break_value = float(break_text)
    except ValueError:
        result_label.config(text="Please enter valid durations.", fg=ERROR)
...
def verify_input():
    ...
    if (work_duration <= 0 or break_duration <= 0 or type(work_duration) != float or type(break_duration) != float
    ...
    sleep(0.1)
    durations_push.forget()
    break_duration_input.forget()
    work_duration_input.forget()
    sleep(0.1)
    result_label.config(text="Ready to start your Pomodoro session!", fg=ACCENT_COLOR)
    sleep(0.1)
    app_state_2()
    sleep(0.5)
...
timer_label = tk.Label(root, text=f"{int(work_duration):02}:00", font=FONT_TIMER, fg=TEXT_COLOR, bg="#000000")
...
def countdown(seconds, is_break=False):
    global timer_id, remaining_time, is_on_break, timer_running
    ...
    if seconds > 0:
        ...
        if is_break:
            ...
            root.after(0, start_timer)  # Add small delay so message is visible
        else:
            result_label.config(text=f"Pomodoro done! Time for a {round(break_duration/60, 1)}-minute break.")
            root.after(0, start_break)  # Add small delay so message is visible
...
def restart_timer():
    global timer_running, timer_id, remaining_time, work_duration, break_duration
    if timer_id:
        root.after_cancel(timer_id)
    timer_running = False
    remaining_time = 0
